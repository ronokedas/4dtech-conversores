...
export function PdfToWordConverter() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/conversions/pdf-word`, { method: "POST", body: data });
  ...
  return <form className="image-tool" onSubmit={submit} aria-busy={busy}>
    ...
    <button className="button primary convert-button" disabled={busy}>{busy ? <><span className="spinner" /> Convertendo...</> : <><Upload size={18} /> Converter para Word <ArrowRight size={19} /></>}</button>
    <p className="privacy-note"><LockKeyhole size={15} aria-hidden="true" /> PDF e Word gerado sao apagados automaticamente apos o download.</p>
  </form>;
}
